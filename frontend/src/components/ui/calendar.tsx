...
function buildClassNames({ defaultClassNames, captionLayout, buttonVariant, showWeekNumber }: { defaultClassNames: ReturnType<typeof getDefaultClassNames>; captionLayout: string; buttonVariant: React.ComponentProps<typeof Button>["variant"]; showWeekNumber: boolean | undefined }) {
  return {
    ...
    day: cn("relative w-full h-full p-0 text-center [&:last-child[data-selected=true]_button]:rounded-r-md group/day aspect-square select-none", showWeekNumber ? "[&:nth-child(2)[data-selected=true]_button]:rounded-l-md" : "[&:first-child[data-selected=true]_button]:rounded-l-md", defaultClassNames.day),
...
function buildComponents(components: React.ComponentProps<typeof DayPicker>["components"]) {
  return {
    ...
    Chevron: ({ className, orientation, ...props }: { className?: string; orientation?: string } & React.SVGAttributes<SVGElement>) => {
...
function Calendar({
  ...
  const builtClassNames = buildClassNames({ defaultClassNames, captionLayout, buttonVariant, showWeekNumber: props.showWeekNumber })
  ...
  return (
    <DayPicker showOutsideDays={showOutsideDays}
      ...
      classNames={{ ...builtClassNames, ...classNames }}
      components={buildComponents(components) as never}
      {...props}
    />
  )
}
...
function CalendarDayButton({ className, day, modifiers, ...props }: React.ComponentProps<typeof DayButton>) {
  ...
  return (
    <Button ref={ref} variant="ghost" size="icon" data-day={day.date.toLocaleDateString()}
      data-selected-single={modifiers.selected && !modifiers.range_start && !modifiers.range_end && !modifiers.range_middle}
      data-range-start={modifiers.range_start} data-range-end={modifiers.range_end} data-range-middle={modifiers.range_middle}
