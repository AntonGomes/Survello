...
default_url = default_client.generate_presigned_url(
    'put_object',
    Params={'Bucket': settings.s3_bucket_name, 'Key': '1/1/test.txt', 'ContentType': 'text/plain'},
    ExpiresIn=3600
)
print(f'Default (no endpoint_url): {default_url.split("?")[0]}')

# Client WITH endpoint_url
endpoint_client = session.client('s3', region_name=settings.aws_default_region, endpoint_url=settings.s3_endpoint_url)
