...
def test_create_run_endpoint(client: TestClient, session: Session, setup_data: dict, mock_storage: MagicMock, mock_llm: MagicMock):
    # Arrange
    payload = {
    ...
    with patch("app.orchestrators.docgen.to_pdf", return_value=b"fake pdf content"):
    ...
    assert db_run.status in ["completed", "error", "uploading", "generating", "finalising"]
