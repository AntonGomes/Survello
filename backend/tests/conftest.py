...
def mock_llm_fixture():
    ...
    mock.upload_template.return_value = LLMContainer(
        "file_id", "container_id", "container_file_id"
    )
...
def client_fixture(
    session: Session, mock_storage: StorageService, mock_llm: BaseLLMService
):
