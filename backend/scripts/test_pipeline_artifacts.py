...
def main():
    ...
    template_path = (
        base_path / "spikes/examples/mvp_test/template/example_final_dilaps.xlsx"
    )
    ...
    try:
        bundle = prepare(
            ...
            on_progress=lambda done, total: print(
                f"[PREPARE PROGRESS] {done * 100 / total}%"
            ),
        )
    ...
    try:
        client_container_bundle = openai_service.upload_bundle(
            ...
            bundle=bundle,
            on_progress=lambda done, total: print(
                f"[UPLOAD PROGRESS] {done * 100 / total}%"
            ),
        )
    except Exception as e:
    ...
    try:
        ...
        print(
            f"Template Container File ID: {client_container_bundle.template_container_file_id}"
        )
    ...
    try:
        ...
        final_doc_path = (
            output_dir / f"{job_id}_final_document.xlsx"
        )  # Assuming xlsx based on template
