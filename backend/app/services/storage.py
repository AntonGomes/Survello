...
class StorageService:
    ...
    def get_file_data(self, storage_key: str) -> bytes:
        obj = self.s3.client.get_object(  # type: ignore
    ...
    def upload_file(self, storage_key: str, data: bytes) -> None:
        self.s3.client.put_object(  # type: ignore
            Bucket=self.s3.bucket_name, Key=storage_key, Body=data
        )

    def check_file_exists(self, storage_key: str) -> bool:
        try:
            self.s3.client.head_object(Bucket=self.s3.bucket_name, Key=storage_key)  # type: ignore
            return True
        except Exception:
            return False
    ...
    def generate_presigned_url(
        ...
        return self.s3.client.generate_presigned_url(  # type: ignore
